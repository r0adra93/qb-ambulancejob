...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("bank", Config.BillCost , "respawned-at-hospital")
		exports['qb-management']:AddMoney(PlayerJob.name, Config.BillCost)
...
RegisterNetEvent('hospital:server:ambulanceAlert', function(text)
    ...
    for _, v in pairs(players) do
        if (v.PlayerJob.name == 'ambulance' or v.PlayerJob.type == 'ems') and v.PlayerData.job.onduty then
...
RegisterNetEvent('hospital:server:UpdateCurrentDoctors', function()
    ...
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "ambulance" or v.PlayerData.job.type == "ems") and v.PlayerData.job.onduty then
            amount += 1
        end
    end
...
RegisterNetEvent('hospital:server:AddDoctor', function(job)
	print('Deprecated use hospital:server:UpdateCurrentDoctors')
end)
